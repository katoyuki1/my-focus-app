...
export default function App() {
  ...
  const [customQuotes, setCustomQuotes] = useState<string[]>([]); // ユーザーが追加した名言リスト
  ...
  useEffect(() => {
    const loadQuotes = async () => {
      try {
        ...
        if (storedQuotes) {
          setCustomQuotes(JSON.parse(storedQuotes)); // JSONを配列に変換
        }
      } catch (error) {
  ...
  }, [isRunning, customQuotes]); // `customQuotes` が変わるたびに再計算
  ...
  const addQuote = async () => {
    if (newQuote.trim() !== "") {
      const updatedQuotes = [...customQuotes, newQuote]; // 新しい名言をリストに追加
      setCustomQuotes(updatedQuotes);
      setNewQuote(""); // 入力欄をクリア

      try {
        await AsyncStorage.setItem("customQuotes", JSON.stringify(updatedQuotes)); // データを保存
      } catch (error) {
